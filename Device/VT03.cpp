 ...
 */
uint16_t VT03::Get_CRC16_Check_Sum(uint8_t *P_Msg, uint16_t Length, uint16_t CRC16)
{
    uint8_t data;
 ...
 */
bool VT03::Verify_CRC16_Check_Sum(uint8_t *P_Msg, uint16_t Length)
{
 ...
 * @param huart 指定的UART
 */
void VT03::Init(UART_HandleTypeDef *huart, Uart_Callback callback_function, uint16_t rx_buffer_length)
{
    uart_init(huart, callback_function, UART_BUFFER_LENGTH);
}
 ...
 * @param Rx_Data 接收的数据
 */
void VT03::UART_RxCpltCallback(uint8_t *Rx_Data, uint16_t Length)
{
    // 滑动窗口, 判断遥控器DR16是否在线
    Flag += 1;
 ...
 *
 */
void VT03::TIM_1ms_Calculate_PeriodElapsedCallback()
{
    // 数据处理过程
    Remote_Data *tmp_buffer = (Remote_Data *) uart_manage_object->rx_buffer;
 ...
 *
 */
void VT03::Data_Process(uint16_t Length)
{
    // 数据处理过程
    Remote_Data *tmp_buffer = (Remote_Data *) uart_manage_object->rx_buffer;
...
//  *
//  */
// void VT03::_Judge_Switch(Enum_VT03_Switch_Status *Switch, uint8_t Status, uint8_t Pre_Status)
// {
//     // 带触发的判断
//     switch (Pre_Status)
//     {
 ...
 *
 */
void VT03::_Judge_Key(VT03_Key_Status *Key, uint8_t Status, uint8_t Pre_Status)
{
    // 带触发的判断
    switch (Pre_Status)
    {
